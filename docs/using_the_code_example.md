...
The following example clones the "[PDM to I2S](https://github.com/Infineon/mtb-example-psoc-edge-pdm-to-i2s)" application with the desired name "MyPdmToI2s" configured for the *KIT_PSE84_EVAL_EPC2* BSP into the specified working directory, *C:/mtb_projects*:

   ```
   project-creator-cli --board-id KIT_PSE84_EVAL_EPC2 --app-id mtb-example-psoc-edge-pdm-to-i2s --user-app-name MyPdmToI2s --target-dir "C:/mtb_projects"
   ```
